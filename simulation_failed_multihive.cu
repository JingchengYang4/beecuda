...
struct hive
{
    ...
    int eggLaid = 0;
    bool shouldSwarm = false;
    int swarm_bees;
    int swam_nutrition;
    int nutrition_last_year = 0;
};
...
__host__ __device__ bool operator<(const brood &a, const brood &b) { return (a.age < b.age); };
...
int hive_cnt = 1;
device_vector<hive> d_hives(100);
host_vector<hive> h_hives(100);
...
__device__ double germination_rate = 0.9;
...
__global__ void plotPollinationUpdate(plot *p, int plot_cnt, hive *h, double efficiency, int day)
{
    ...
    if(index < plot_cnt)
    {
        ...
        if(p[index].pollinated_today > p[index].flowers_available)
        {
            ...
            h[index].total_nutrition -= (int)efficiency * (p[index].pollinated_flowers - p[index].total_flowers)/26;
            //I see
...
__global__ void hiveUpdate(hive *h, int hive_cnt, int day, plot *p, curandState *randstate)
{
    int index = threadIdx.x + blockIdx.x * blockDim.x;
    if(index < hive_cnt)
    {
        ...
        h[index].eggLaid = ((p[0].flowers_available / 20000000000000)+1) * eggLayingRate(day);// / hive_cnt;
        ...
        //h[index].shouldSwarm = true;
       // h[index].shouldSwarm = true;

        if(day == 190)
        {
            h[index].shouldSwarm = true;
            if(h[index].eggLaid > 1500)
            {
                if(abs(h[index].nutrition_last_year - h[index].total_nutrition) < 2500)
                {
                    //stabilized enough for swarming
                    //if random that is...
                    h[index].shouldSwarm = true;
                    if(curand_uniform(&(randstate[index])) <= 0.4)
                    {
                        //swarming
                        h[index].shouldSwarm = true;
                    }
            ...
            h[index].nutrition_last_year = h[index].total_nutrition;
        }
...
struct bee_hive_filter_functor : thrust::unary_function<bee, bool>
{
    const hive* hive_ptr;

    bee_hive_filter_functor(hive* _a) : hive_ptr(_a) {}

    __host__ __device__
    bool operator()(const bee &b)
    {
        return b.hive_ptr == hive_ptr;
    }
};

struct bee_hive_not_filter_functor : thrust::unary_function<bee, bool>
{
    const hive* hive_ptr;

    bee_hive_not_filter_functor(hive* _a) : hive_ptr(_a) {}
...
struct brood_hive_filter_functor : thrust::unary_function<brood, bool>
{
    const hive* hive_ptr;

    brood_hive_filter_functor(hive* _a) : hive_ptr(_a) {}

    __host__ __device__
    bool operator()(const brood &b)
    {
        return b.hive_ptr == hive_ptr;
    }
};

struct brood_hive_not_filter_functor : thrust::unary_function<brood, bool>
{
    const hive* hive_ptr;

    brood_hive_not_filter_functor(hive* _a) : hive_ptr(_a) {}
...
struct bee_hive_change_functor
{
    hive* hive_ptr;

    bee_hive_change_functor(hive* _a) : hive_ptr(_a) {}

    __host__ __device__
        bee operator()(bee& b) const 
        {
            b.hive_ptr = hive_ptr;
            return b;
        }
...
struct brood_mature_functor : thrust::unary_function<brood, bool>
{
    ...
    brood_mature_functor(hive* _a) : hive_ptr(_a) {}

    __host__ __device__
    bool operator()(const brood &x)
    {
        return x.age < 21 && x.hive_ptr != hive_ptr;
    }
};
...
int main(int argc, char** argv)
{
    ...
    int brood_cnt = 1500;
    int matured_brood = 0;
    thrust::fill(d_broods.begin(), d_broods.begin() + brood_cnt, brood(&d_hive_array[0], 0));
    ...
    device_vector<bee> hive_bees(beeMax);
    device_vector<brood> hive_broods(beeMax);
    ...
    for(int day = 0; day < simulationDays; day++)
    {
        ...
        hiveUpdate<<<(hive_cnt + threadsPerBlock-1)/threadsPerBlock, threadsPerBlock>>>(d_hive_array, hive_cnt, day%365, d_plot_array, d_state);
        cudaDeviceSynchronize();

        h_hives = d_hives;

        bool swarmModified = false;

        for(int i = 0; i < hive_cnt; i++)
        {
            if(h_hives[i].shouldSwarm)
            {

                cout << "SWARMED" << endl;
                //change half of the bees
                //add it to the list
                swarmModified = true;
                detail::normal_iterator<device_ptr<bee>> hive_bees_end = thrust::copy_if(d_bees.begin(), d_bees.begin() + bee_cnt, hive_bees.begin(), bee_hive_filter_functor(&d_hive_array[i]));
                int hive_bees_cnt = hive_bees_end - hive_bees.begin();

                int newHiveIndex = hive_cnt;
                hive_cnt++;
                h_hives[newHiveIndex].total_bees = hive_bees_cnt/2;
                h_hives[newHiveIndex].total_nutrition = h_hives[i].total_nutrition * 4;
                h_hives[i].total_nutrition *= 4;//-= h_hives[newHiveIndex].total_nutrition;
                h_hives[i].total_bees -= h_hives[newHiveIndex].total_bees;

                thrust::sort(hive_bees.begin(), hive_bees_end);

                thrust::transform(hive_bees.begin(), hive_bees.begin() + h_hives[newHiveIndex].total_bees, hive_bees.begin(), bee_hive_change_functor(&d_hive_array[newHiveIndex]));

                detail::normal_iterator<device_ptr<bee>> master_bee_end = thrust::copy_if(d_bees.begin(), d_bees.begin() + bee_cnt, d_bees.begin(), bee_hive_not_filter_functor(&d_hive_array[i]));
                
                thrust::merge(hive_bees.begin(), hive_bees_end, d_bees.begin(), master_bee_end,  d_bees.begin());

                h_hives[i].shouldSwarm = false;
            }
        }
        ...
        for(int i = 0; i < hive_cnt; i++)
        {
            if(i == 0) outValue("HC", hive_cnt);

            cudaMemcpy(&o_hive, &d_hive_array[i], sizeof(hive), cudaMemcpyDeviceToHost);

            if(i == 0) outValue("Net", o_hive.nutrition_deficit);

            //int brood_die = (int)o_hive.should_die/2;

            //if(i == 0) outValue("SD", o_hive.should_bee_die);
            ...
            //Fix it here
            detail::normal_iterator<device_ptr<bee>> hive_bees_end = thrust::copy_if(d_bees.begin(), d_bees.begin() + bee_cnt, hive_bees.begin(), bee_hive_filter_functor(&d_hive_array[i]));
            int originalCount = hive_bees_end - hive_bees.begin();
            hive_bees_end -= min((int)o_hive.should_bee_die, (int)(hive_bees_end - hive_bees.begin()));

            if(i == 0) outValue("D", (int)(originalCount - (hive_bees_end - hive_bees.begin())));

            detail::normal_iterator<device_ptr<bee>> master_bee_end = thrust::copy_if(d_bees.begin(), d_bees.begin() + bee_cnt, d_bees.begin(), bee_hive_not_filter_functor(&d_hive_array[i]));
            thrust::merge(hive_bees.begin(), hive_bees_end, d_bees.begin(), master_bee_end,  d_bees.begin());

            bee_cnt = (master_bee_end - d_bees.begin()) + (hive_bees_end - hive_bees.begin());


            //666
            //if(hive_cnt > 1) return;

            if(i == 0) outValue("S", o_hive.should_bee_die + o_hive.should_brood_die);

            detail::normal_iterator<device_ptr<brood>> hive_broods_end = thrust::copy_if(d_broods.begin(), d_broods.begin() + brood_cnt, hive_broods.begin(), brood_hive_filter_functor(&d_hive_array[i]));
            int originalBroodCnt = hive_broods_end - hive_broods.begin();

            hive_broods_end -= min(o_hive.should_brood_die + o_hive.unattained_broods, (int)(hive_broods_end - hive_broods.begin()));

            detail::normal_iterator<device_ptr<brood>> master_brood_end = thrust::copy_if(d_broods.begin(), d_broods.begin() + brood_cnt, d_broods.begin(), brood_hive_not_filter_functor(&d_hive_array[i]));
            
            thrust::merge(hive_broods.begin(), hive_broods_end, d_broods.begin(), master_brood_end,  d_broods.begin());

            brood_cnt = (master_brood_end - d_broods.begin()) + (hive_broods_end - hive_broods.begin());

            //brood_cnt -= min(o_hive.should_brood_die + o_hive.unattained_broods, brood_cnt);
            //brood_cnt -= min(o_hive.unattained_broods, brood_cnt);
        
            if(i == 0) outValue("U", o_hive.unattained_broods);

            detail::normal_iterator<device_ptr<brood>> unmatured_bee_end = thrust::copy_if(d_broods.begin(), d_broods.begin()+brood_cnt, d_broods.begin(), brood_mature_functor(&d_hive_array[i]));
            matured_brood =(d_broods.begin() + brood_cnt) - unmatured_bee_end;
            matured_brood = int(matured_brood * (1-brood_mature_death_rate));
            if(i == 0) outValue("M", matured_brood);

            //int obc = bee_cnt;
            
            // if(day > 673) return;
            addRatioBee(bee(&d_hive_array[i], 0, 1, 0), matured_brood);

            //if(i == 0) outValue("NC", bee_cnt - obc);

            brood_cnt = unmatured_bee_end - d_broods.begin();

            if(i == 0)  outValue("N", o_hive.total_nutrition);

            //lay eggs
            int eggLaid = o_hive.eggLaid;
            if(i == 0) outValue("Egg", eggLaid);

            thrust::fill(unmatured_bee_end, unmatured_bee_end+eggLaid, brood(&d_hive_array[i], 0));
            brood_cnt+=eggLaid;
