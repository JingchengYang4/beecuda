...
//statics
__device__ int broodConsumption = 6;
__device__ int beeConsumption = 2;
...
double foraging_multiplier = 1;
__device__ int flower_pollinated_cnt = 50;
//
int bc = 2;
int pc = 6;
...
__device__ double seed_per_flower = 20;
...
__global__ void plotPollinationUpdate(plot *p, int plot_cnt, hive *h, double efficiency, int day)
{
    ...
    if(index < plot_cnt)
    {
        if(day >= 50 && day <= 100)
        {
            unsigned long long blossomedFlowers = (p[index].pollinated_last_year*seed_per_flower) * germination_rate;
            p[index].pollinated_last_year -= p[index].pollinated_last_year/(101-day);
        ...
        if(p[index].pollinated_today > p[index].flowers_available)
        {
            ...
            h[index].total_nutrition -= (int)efficiency * (p[index].pollinated_flowers - p[index].total_flowers)/flower_pollinated_cnt;
            //I see
...
__global__ void hiveUpdate(hive *h, int hive_cnt, int day, plot *p, curandState *randstate)
{
    ...
    if(index < hive_cnt)
    {
        ...
        h[index].eggLaid = ((p[0].flowers_available / 20000000000000)+1) * eggLayingRate(day);
...
__global__ void beeForageUpdate(bee *b, int bee_cnt, plot *p, double efficiency, double lifespan)
{
    ...
    if(index < bee_cnt)
    {
        if(b[index].type == 0 && b[index].age >= 0.5*lifespan)
        {
            ...
            atomicAdd(&(*p).pollinated_today, flower_pollinated_cnt*efficiency/8.57087);
...
int main(int argc, char** argv)
{
    ...
    for(int i = 1; i < argc; i++)
    {
        ...
        if(!strcmp(argv[i], "-sf"))
        {
            if(i+1 < argc)
            {
                double sf = atof(argv[i+1]);
                cudaMemcpyToSymbol(seed_per_flower, &sf, sizeof(double), 0, cudaMemcpyHostToDevice);
            }
        }
        ...
        if(!strcmp(argv[i], "-fm"))
        {
            if(i+1 < argc)
            {
                foraging_multiplier = atof(argv[i+1]);
            }
        }
    }
    ...
    for(int day = 0; day < simulationDays; day++)
    {
        ...
        double efficiency = forageEfficiency(day%365) * foraging_multiplier;
